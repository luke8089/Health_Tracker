...
// Check login status before allowing access to protected pages
function checkLoginStatus(page) {
  ...
  fetch(basePath + '/public/api/check_session.php')
    .then(response => response.json())
    .then(data => {
      if (data.logged_in) {
        // User is logged in, redirect to the requested page
        window.location.href = basePath + `/public/${page}.php`;
      } else {
        // User is not logged in, redirect to login page
        window.location.href = basePath + '/public/login.php';
      }
    })
    .catch(error => {
